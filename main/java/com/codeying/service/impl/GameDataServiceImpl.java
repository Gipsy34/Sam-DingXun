...
import com.codeying.mapper.GameDataMapper;
...
/** 服务实现类 */
@Service
public class GameDataServiceImpl extends AbsServiceImpl<GameDataMapper, GameData>
    implements GameDataService {
  ...
  public List<GameData> recommend(List<GameData> all) {
    ...
    final List<GameData> itemList;
    Map<String, GameData> collect = all.stream().collect(Collectors.toMap(GameData::getId, e -> e));
    List<Star> allStar =
        starService.list(
            new LambdaQueryWrapper<Star>()
                .eq(Star::getItemtype, "gameData")
    ...
    if (loginUser.getRole().equalsIgnoreCase("User")) {
      ...
      userLabels = thisUser.getBiaoq();
      if (userLabels != null && !userLabels.isEmpty()) {
        for (GameData value : collect.values()) {
          int similarity = AdvancedTextSimilarity.calculateSimilarity(userLabels, value.getBiaoq());
          if (similarity > 0) {
    ...
    StringBuilder myStarLabels = null; // 我Favorite的所有标签
    for (String myStarId : myStarIds) {
      GameData value = collect.get(myStarId);
      if (value == null) {
        continue;
      }
      if (value.getBiaoq() != null && !value.getBiaoq().isEmpty()) {
        if (myStarLabels == null) {
          myStarLabels = new StringBuilder();
          myStarLabels.append(value.getBiaoq());
        } else {
          myStarLabels.append(",").append(value.getBiaoq());
        }
      }
    }
    ...
    if (myStarLabels != null) {
      for (GameData value : collect.values()) {
        if (myStarIds.contains(value.getId())) {
          ...
          int similarity =
              AdvancedTextSimilarity.calculateSimilarity(myStarLabels.toString(), value.getBiaoq());
          if (similarity > 0) {
            // 命中Favorite标签
  ...
  @Override
  public List<GameData> sqlSelectList(GameData qo) {
  ...
  @Override
  public int sqlUpdate(GameData e) {
    return baseMapper.sqlUpdate(e);
  }

  @Override
  public int sqlSave(GameData e) {
    return baseMapper.sqlSave(e);
  }

  @Override
  public List<GameData> topN(int n) {
    Page<GameData> page = new Page<>(1, n);
